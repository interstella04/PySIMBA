...
class Theory:
    ...
    def FullBsgPrediction(
        self, key: str, end: str, c_n_params: npVector, norm: float
    ) -> npVector:
        ...
        for order in range(np.size(settings.TheoryOrder)):
            pred += self.BsgPrediction(
                key, settings.TheoryOrder[order], end, c_n_params, norm
            ) * self.TheoryPrefactor(settings.TheoryOrder[order], norm)

        # Full subleading prediction
        for order in range(np.size(settings.SubLeadTheoryOrder)):
            pred += self.BsgSubLeadingPrediction(
                key,
                self.SubLeadPars(c_n_params, settings.SubLeadCoefficients[order]),
                norm,
            ) * self.TheoryPrefactor(settings.SubLeadTheoryOrder, norm)
    ...
    def BsgPrediction(
        self, key: str, mid: str, end: str, c_n_params: npVector, norm: float
    ) -> npVector:
        pred = np.array([])
        for i in range(np.size(self.ExpData[key]["dBFs"])):
            ...
            for j in range(np.size(c_n_params)):
                for k in range(np.size(c_n_params)):
                    value += (
                        self.Theory[key][mid]["la" + end]["Values"][i][j][k]
                        * c_n_params[j]
                        * c_n_params[k]
                    )
    ...
    def BsgSubLeadingPrediction(
        self, key: str, c_n_params: npVector, norm: float
    ) -> npVector:
        pred = np.array([])
        for i in range(np.size(self.ExpData[key]["dBFs"])):
            value = 0
            for j in range(np.size(c_n_params)):
                value += (
                    self.SubleadTheory[key]["la" + settings.SubLeadBasisExpansion][
                        "Values"
                    ][i][0][j]
                    * c_n_params[j]
                )
    ...
    def SubLeadPars(self, c_n_params: npVector, d2: float) -> npVector:
        ...
        if "SSF27_1" in settings.SubLeadTheoryOrder:
            x = (
                0.6514810199386504 * (mB - mb)
                - 0.8686413599182005 * la
                + 0.3257405099693252 * Rho2 / Lambda2
            ) / (1.8531015678254943 * la - d2 * la)
        elif "SSF27_2" in settings.SubLeadTheoryOrder:
            x = (
                0.4722982832332954 * (mB - mb)
                - 0.5667579398799545 * la
                + 0.2361491416166477 * Rho2 / Lambda2
            ) / (1.3695121740357048 * la - d2 * la)
        else:
            print(
                "Unknown subleading shape function -- please specify Theory.SubLeadPars()"
            )

        # calculating d0, d1 and d2 and returning them in a numpy array
        return np.array([1.0 - x, x * (1 - d2), x * d2])

    # Calculate shape fuction moment of order 'order'
    def Moment(self, c_n_params: npVector, order: int) -> float:
        if order > np.size(self.FmnMoments[settings.config["TheoryTag"]]["Moment"]):
            print(
                "Theory.Moment(): requested an order of moment, that cannot be calculated"
            )
        value = 0

        # Prepares Moment, using the lambda for which the fit is currently calculated
        prepared_moment = (
            np.pow(Tools.StrToLambda(settings.SubLeadBasisExpansion), order)
            * self.FmnMoments[settings.config["TheoryTag"]]["Values"][order]
        )
    ...
    def mb1SPrediction(self, c_n_params: npVector) -> float:
        ...
        q = (
            5 * M1 * (27 * Lambda2 + 25 * M1 * M1 - 54 * M2)
            - 3 * (45 * Lambda2 - 55 * M1 * M1 + 72 * M2) * mB
            + 51 * M1 * mB * mB
            - 17 * mB * mB * mB
            + 162 * (M3 + Rho2)
        )
        u = np.cbrt((-q + np.sqrt(q * q + p * p * p)))
        mb = 1.0 / 6.0 * (5 * (mB - M1) - p / u + u)
    ...
    def lambda11SPrediction(self, c_n_params: npVector) -> float:
        ...
        mb = self.mb1SPrediction(c_n_params)
        lambda1 = 3.0 * lambda2 + 2.0 * mb * (mB - M1 - mb)
    ...
    def Chisq(self, par: Vector) -> float:
        ...
        for key in settings.KeyOrder:
            ...
            full_pred = Theory.FullBsgPrediction(
                self, key, settings.BasisExpansion, cn, norm
            )
        ...
        Cov_glob = Tools.MakePositiveDefinite(
            Cov_glob, eps=0
        )  # Forces Matrice to be positive definite
